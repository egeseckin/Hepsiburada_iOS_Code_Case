...
class iTunesCollectionViewCell: UICollectionViewCell {
    ...
    func setup(with items: [iTunesApiData] ){
        let imageUrl = URL(string: items[0].results[0].artworkUrl100)!
        let imageData = try! Data(contentsOf: imageUrl)
        self.iTunesImage.image = UIImage(data: imageData)
        
        self.nameLabel.text = "Name: " + items[0].results[0].collectionName
        self.priceLabel.text = "Price: " + String(items[0].results[0].collectionPrice) + "$"
        
        let date = items[0].results[0].releaseDate.components(separatedBy: "T")
